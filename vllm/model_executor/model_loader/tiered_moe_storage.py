...
GLM_MARLIN_COMPONENTS = (
    ...
    ExpertComponentSpec("w13_weight_scale", (48, 4096), torch.bfloat16),
    ExpertComponentSpec("w2_weight_scale", (16, 6144), torch.bfloat16),
...
def build_expert_component_views(
    ...
    required_bytes = expert_count * GLM_MARLIN_EXPERT_BYTES
...
class ExpertTierStorage:
    ...
    def copy_expert_from(
        ...
        expected_names = {component.name for component in GLM_MARLIN_COMPONENTS}
...
def allocate_layer_expert_storage(
    ...
    if placement.hot_expert_ids:
        hot_bytes = len(placement.hot_expert_ids) * GLM_MARLIN_EXPERT_BYTES
        ...
        hot = ExpertTierStorage(
            ...
            components=build_expert_component_views(
                hot_buffer, len(placement.hot_expert_ids)
            ),
        )

    cold = None
    if placement.cold_expert_ids:
        cold_bytes = len(placement.cold_expert_ids) * GLM_MARLIN_EXPERT_BYTES
        ...
        cold = ExpertTierStorage(
            ...
            components=build_expert_component_views(
                allocation.cuda_alias, len(placement.cold_expert_ids)
            ),
