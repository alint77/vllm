...
ReplicaAssignment = Literal["off", "secondary", "exact"]
...
class TieredMoEConfig:
    ...
    """Replica assignment: primary-only, static secondary, or exact decode.

    `exact` minimises the maximum per-rank count of active cold experts by
    exact min-max edge orientation, which conserves total Grace work.
    """
